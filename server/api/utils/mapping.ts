  ...
  mapFinancialItems(
    ...
    const budgetToFinancials = budgetData.map((budget: any) => {
      ...
      if (
        ...
        budgetLevel =
          reportingSubmission?.financialData?.dofp?.overhead?.budget ?? {};
      } else if (
        budget.expenseCategory === "Division Of Family Practice" &&
        budget.expenseItem === "Change Management"
      ) {
        budgetLevel =
          reportingSubmission?.financialData?.changeManagement?.budget ?? {};
      } else if (
        ...
        if (initiative === "pcn") {
          budgetLevel =
            reportingSubmission?.financialData?.healthAuthority?.overhead
              ?.budget ?? {};
        } else {
          budgetLevel =
            reportingSubmission?.financialData?.overhead?.budget ?? {};
        }
      }
          ...
          .filter((report: any) => {
            if (
              ...
              if (
                ...
                initiative === "pcn" &&
                budget.expenseItem === "Change Management" &&
                report.expenseItem === "Change Management"
              ) {
                return true;
              } else if (
                budget.expenseItem === report.expenseItem &&
                !budget.expenseItemSubType &&
                !report.expenseItemSubType &&
                (initiative === "upcc"
                  ? budget.typeOfCare === report.typeOfCare
                  : true)
              ) {
